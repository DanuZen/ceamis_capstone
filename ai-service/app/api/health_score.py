...
NOTE: Jika model artifact belum tersedia di app/models/,
endpoint akan return HTTP 503 (Service Unavailable).
Gunakan /api/v1/health-score/info untuk cek status model.
"""
...
# ─── Custom components (wajib didaftarkan agar model bisa di-load) ────────────
...
async def health_score_info():
    """Informasi model, threshold Warning System, dan status artifact."""
    model_ready  = MODEL_PATH.exists()
    scaler_ready = SCALER_PATH.exists()

    return {
        "model":            "financial_health_model.keras",
        "model_ready":      model_ready,
        "scaler_ready":     scaler_ready,
        "status":           "ready" if (model_ready and scaler_ready) else "artifact_missing",
